...
func (r Running) Calories() float64 {
	// вставьте ваш код ниже
	meanSpeedMinSec := r.meanSpeed() * float64(KmHInMsec)
	squareMeanSpeed := meanSpeedMinSec * r.Training.meanSpeed()
	weight := r.Training.Weight
	timeHours := r.Duration.Hours()
	return (CaloriesMeanSpeedMultiplier*squareMeanSpeed + CaloriesMeanSpeedShift) * weight / MInKm * timeHours * MinInHours
}
...
const (
	...
	KmHInMsec                     (float64) = 0.278 // коэффициент для перевода км/ч в м/с
)
...
func (w Walking) Calories() float64 {
	// вставьте ваш код ниже
	meanSpeedMinSec := w.Training.meanSpeed() * KmHInMsec
	...
	height := w.Height / 100
	timeHours := w.Training.Duration.Hours()
	return ((CaloriesWeightMultiplier*weight + (squareMeanSpeed/height)*CaloriesSpeedHeightMultiplier*weight) * timeHours * MinInHours)
}
...
func (s Swimming) Calories() float64 {
	// вставьте ваш код ниже
	meanSpeedMinSec := s.meanSpeed() * KmHInMsec
	squareMeanSpeed := meanSpeedMinSec * s.Training.meanSpeed()
	weight := s.Training.Weight
	timeHours := s.Duration.Hours()
	return ((squareMeanSpeed + SwimmingCaloriesMeanSpeedShift) * SwimmingCaloriesWeightMultiplier * weight * timeHours)
}
...
func ReadData(training CaloriesCalculator) string {
	...
	calorie := newFunction(training)
	// получаем информация о тренировке
	info := training.TrainingInfo()
	// добавьте полученные калории в структуру с информацией о тренировке
	//info = InfoMessage{}
	info.Calories = calorie
	//Training.TrainingInfo()
	return fmt.Sprint(info)
}

func newFunction(training CaloriesCalculator) float64 {
	calorie := training.Calories()
	return calorie
}
